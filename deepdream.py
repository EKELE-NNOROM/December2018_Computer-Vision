...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan  9 14:33:54 2019
...
# Download data for inception model if it doesn't exist
...
def save_image(image, filename):
    # Clip image to contain values from 0 to 255
    image = np.clip(image, 0.0, 255.0)
    
    # PIL library only supports 8 bit bytes
    # Convert the pixels to 8-bit bytes
    image = image.astype(np.uint8)
    
    with open(filename, 'wb') as fhand:
        PIL.Image.fromarray(image).save(fhand, 'jpeg')

def plot_image(image):
    
    if False:
        # Convert pixel-values to range from 0.0 to 1.0
...
def resize_image(image, size=None, factor=None):
    if factor is not None:
        size = np.array(image.shape[:2]) * factor
        
        # PIL requires size to be int but size is currently floating-point
    ...
    # Create PIL-Object from numpy as array
...
def optimize_image(layer_tensor, image, 
    ...
    for i in range(num_iterations):
        grad = tiled_gradient(gradient, image=img)
...
def recursive_optimize(layer_tensor, image,
    ...
    if num_repeats>0:
        ...
        img_result = recursive_optimize(layer_tensor=layer_tensor, image=img_downscaled,
                       num_repeats=num_repeats-1, rescale_factor=rescale_factor, blend=blend,
